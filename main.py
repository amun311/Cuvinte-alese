...
class Code_show():
    ...
    def code_minus_click(self, code_value):
        ...
        if self.code_value.value not in self.lista_caractere :
            ...
            if self.code_value.value is not None and self.lista_caractere.index(self.code_value.value.upper()) > 0:
...
def main(page: ft.Page):
    ...
    def game(lb,mod,lb_cuv):
        start=time.time()
        globals()['count'] = 0
        #page.splash = ft.Row([ft.Text(f'',color=ft.Colors.ORANGE,size=20,text_align=ft.TextAlign.CENTER)])
        ...
        try:
            ...
            choice,definitie,nr_cuv = alege_cuvantul(x[1],lb_cuv) #game(lb,mod,lb_cuv)
        ...
        '''def menu_item1(*args):            
            ...
            modal=False,
            title=ft.Text(lang[3], text_align=ft.TextAlign.CENTER,),
        ...
        def menu_item2(*args):
            ...
            dlg_modal = ft.AlertDialog(
                modal=False,
                title=ft.Text(lang[26], text_align=ft.TextAlign.CENTER,),
        ...
        def menu_item3(*args):
            ...
            dlg_modal = ft.AlertDialog(
                modal=False,
                title=ft.Text(lang[1], text_align=ft.TextAlign.CENTER,),
                content=ft.ResponsiveRow([ft.Text(
                    spans=[
                        ...
                        ft.Row([ft.IconButton(ft.Icons.HAIL,disabled=True,disabled_color='white'),ft.Text(lang[31], text_align=ft.TextAlign.CENTER,)],ft.MainAxisAlignment.CENTER),
        ...
        def menu_item4(*args):
            ...
            dlg_modal = ft.AlertDialog(
                modal=False,
                title=ft.Text(lang[2], text_align=ft.TextAlign.CENTER,),
                on_dismiss= close_dlg,
                content=ft.Text(spans= [ft.TextSpan('Made with ❤️ by Alexandru G. Muntenas\nfor my son and my wife\n'), ft.TextSpan('alexandru.muntenas@gmail.com',on_click=lambda _:page.launch_url('mailto:alexandru.muntenas@gmail.com'),style=ft.TextStyle(color='red',weight=ft.FontWeight.W_400))], size=14,italic = True, text_align=ft.TextAlign.CENTER),
        ...
        def half_choice(e):
            ...
            half_choice_cont.content = ft.Text(value = f'{lang[1]}: {rand_choice.strip(" ,")}',color=ft.Colors.GREEN,size=20,text_align=ft.TextAlign.CENTER)
            #page.overlay.append(ft.Row([ft.Text(choice[:len(choice)//2+1])]))
            globals()['vieti'] -= globals()['vieti']//2 #(len(choice)//2+1)*2
            cont_count.content = ft.Text(f'{lang[22].capitalize()}: {vieti}',color=ft.Colors.ORANGE,size=20,text_align=ft.TextAlign.CENTER)
            cont_count.update()
        ...
        (slider_value := ft.Text(f"{lang[11].capitalize()} {x[1]} {lang[12]}",color=ft.Colors.ORANGE,size=20,text_align=ft.TextAlign.CENTER))
        ...
        page.add(ft.Column([ft.Row([cont_count, ft.IconButton(ft.Icons.HAIL,on_click = half_choice, tooltip=lang[31],icon_color='green'),ft.Text(expand=True),
                                    ...
                                    ft.Text('')]),
                                    ft.Row([half_choice_cont,ft.Text(expand=True),slider_value,ft.Text(str(nr_cuv), color = ft.Colors.RED,size=20)],alignment=ft.MainAxisAlignment.END)
        ...
        def verifica(code):
            ...
            def check (choice ,code,pos): 
                if code[pos] not in lista_caractere:
                    checked = ft.TextField(value=lang[12].capitalize(), text_align=ft.TextAlign.CENTER,height=60,border_width=5,border_radius=40,border_color='white',tooltip=lang[12],read_only=True,col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)},)
                       
                elif code[pos]==choice[pos]:
                    checked = ft.TextField(value=code[pos], text_align=ft.TextAlign.CENTER,height=60,border_width=5,border_radius=40,border_color='green',tooltip=lang[6],read_only=True,col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)},)
                    ...
                    checked= ft.TextField(value=code[pos], text_align=ft.TextAlign.CENTER,height=60,border_width=5,border_radius=40,border_color='blue',tooltip=lang[8],read_only=True,col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)},) 
                elif code[pos] in choice :
                    checked = ft.TextField(value=code[pos], text_align=ft.TextAlign.CENTER, height=60,border_width=5,border_radius=40,border_color='orange',tooltip=lang[7],read_only=True,col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)},) 
                else:
                    checked = ft.TextField(value=code[pos], text_align=ft.TextAlign.CENTER,height=60,border_width=5,border_radius=40,border_color='red',tooltip=lang[9],read_only=True,col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)},)
                return checked 
            res = []
            fel=0
        ...
        def increment():
            ...
            #splash = ft.Row([ft.Text(f'{lang[22].capitalize()}: ',color=ft.Colors.ORANGE,size=20,text_align=ft.TextAlign.CENTER),cont_count.content])
        ...
        def code_check(*args):
            ...
            if vieti == 0:
                def close_dlg(*args):  
                    ...
                    globals()['count'] = 0
                    game(lb,mod,lb_cuv)                                                   
                no_dlg = ft.AlertDialog(modal =True,title=ft.Text(lang[14], text_align=ft.TextAlign.CENTER,size=25),
                    content=ft.Text(spans=[ft.TextSpan(f'{lang[15]} {lang[16]}.\n{lang[17]} '),
                                           ft.TextSpan(choice,style=ft.TextStyle(color=ft.Colors.RED)),
                                           ft.TextSpan(f'\n {definitie}')], text_align=ft.TextAlign.CENTER),
                    actions=[ft.OutlinedButton('Ok', on_click=close_dlg)],
                    actions_alignment=ft.MainAxisAlignment.CENTER,
                    on_dismiss= close_dlg,
                    shape=ft.ContinuousRectangleBorder(300),
        ...
        def open_dlg_modal(*args):
            ...
            if count != 1:           
                msg =ft.Text(spans=[ft.TextSpan(f'{lang[18]} {count} {lang[22]}.\n{lang[17]} '),
                                                           ft.TextSpan(choice,style=ft.TextStyle(color=ft.Colors.RED)),
                                                           ft.TextSpan(f'\n⌛ {minute:02.0f}:{secunde:2.0f}')], text_align=ft.TextAlign.CENTER)
            ...
            dlg_modal = ft.AlertDialog(modal=True,title=ft.Text(lang[23], text_align=ft.TextAlign.CENTER,),
                                       content=msg,
                actions=[ft.OutlinedButton('Ok', on_click=close_dlg),],
                actions_alignment=ft.MainAxisAlignment.CENTER,
        ...
        lista_cont_probe = ft.Container(content=ft.ListView(spacing=1), col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)}) #243
        lista_cont_probe = [ft.Container(content=ft.ListView(spacing=1), col={"xs": 12/len(choice), "md": 12/len(choice), "xl":12/len(choice)}) for i in range(len(choice))]#243
        for i in range(len(choice)):  
    ...
    '''try:
        ...
        def close_dlg_ro(*args):                
                ...
                page.update()
                game('ro', 'cuvant5', 'ro')         
        msg =ft.Text(f'Limba / Idioma', text_align=ft.TextAlign.CENTER)
        dlg_modal = ft.AlertDialog(modal=True,title=ft.Text(f'Limba / Idioma', text_align=ft.TextAlign.CENTER,),content=msg,
